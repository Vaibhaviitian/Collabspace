...
const Dashboard = () => {
  ...
  const user = {
    ...
    avatar: `https://ui-avatars.com/api/?name=${localStorage.getItem("username")}&background=0ea5e9&color=fff&rounded=true&size=128`
  };
    ...
    try {
      ...
      allDocsResponse?.data?.data.forEach((item) => {
        item.collaborators.forEach((req_id) => {
          if (req_id.user.toString() === user_id && req_id.permission === "edit") {
            updatedSharedProjects.push({ ...item, action: "edit" });
          } else if (req_id.user.toString() === user_id && req_id.permission === "view") {
  ...
  const filteredProjects = activeTab === "myProjects" 
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 p-6">
      ...
      >
        <button
          ...
          className={`px-6 py-3 font-medium relative ${activeTab === "myProjects" ? 'text-cyan-400' : 'text-gray-400 hover:text-white'}`}
        >
          My Projects
        ...
        <button
          ...
          className={`px-6 py-3 font-medium relative ${activeTab === "sharedProjects" ? 'text-cyan-400' : 'text-gray-400 hover:text-white'}`}
        >
          Shared with Me
                ...
                >
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      ...
                      {activeTab === "sharedProjects" && (
                        <span className={`text-xs px-2 py-1 rounded-full ${project.action === "edit" ? 'bg-green-900 text-green-300' : 'bg-blue-900 text-blue-300'}`}>
                    ...
                    {activeTab === "myProjects" ? (
                      <>
                        <motion.button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/api/new/FluxDocs/${project._id}`, { state: { action: "edit" } });
                          }}
                      ...
                      <motion.button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(`/api/new/FluxDocs/${project._id}`, { state: { action: project.action } });
                        }}
                        className={`text-sm px-4 py-1 rounded-md ${project.action === "edit" ? 'bg-cyan-600' : 'bg-gray-700'}`}
                      >
